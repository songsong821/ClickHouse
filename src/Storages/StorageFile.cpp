...
void ReadFromFile::initializePipeline(QueryPipelineBuilder & pipeline, const BuildQueryPipelineSettings & build_settings)
{
    ...
    /// A single-file split creates one source per bucket, and `computeBucketsByCount` never
    /// returns more buckets than the `max_num_streams` it is asked for, so `output_ports` is
    /// still bounded by `max_num_streams` here and the comparison stays a fan-out test.
