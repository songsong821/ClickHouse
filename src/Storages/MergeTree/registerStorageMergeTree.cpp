...
- Follower: writes fail with `TABLE_IS_READ_ONLY` — this covers `INSERT`, mutations, partition operations (`DETACH`/`ATTACH`/`MOVE`/`REPLACE PARTITION`), `OPTIMIZE`, `FREEZE`/`UNFREEZE`, and `COMMENT TABLE` / `COMMENT COLUMN`. DDL that is unsupported under `leader_election` altogether — every other `ALTER`, `RENAME TABLE` and `RENAME DATABASE` — is rejected earlier and on **every** node, the leader included, with `SUPPORT_IS_DISABLED` (see the list of unsupported operations below); a follower never reports `TABLE_IS_READ_ONLY` for those. `SELECT` is allowed. Background write tasks are stopped, and any in-flight merges or moves on a node that just lost leadership are actively cancelled to bound the dual-writer window.
