...
- Shared metadata: every disk in the storage policy must use `metadata_type = plain_rewritable`. It is a metadata layout where the directory of parts is visible to every participating instance, so the next leader after a failover sees all parts written by the previous leader. Every other metadata layout is rejected at table creation, including the default `metadata_type = local`, under which each replica's part list would be invisible to its peers.
...
- Follower: writes fail with `TABLE_IS_READ_ONLY` — this covers `INSERT`, partition operations (`DETACH`/`ATTACH`/`MOVE`/`REPLACE PARTITION`), `OPTIMIZE`, `FREEZE`/`UNFREEZE`, and `COMMENT TABLE` / `COMMENT COLUMN`. Mutations are not in that list on the currently supported layout: as on the leader, `ALTER TABLE ... UPDATE`/`DELETE` on `plain_rewritable` is rejected with `SUPPORT_IS_DISABLED` by `MergeTreeData::checkMutationIsPossible`, which runs before the read-only fence, so a follower never reports `TABLE_IS_READ_ONLY` for a mutation there; the fence would only apply on a shared layout that does provide hard links. DDL that is unsupported under `leader_election` altogether — every other `ALTER`, `RENAME TABLE` and `RENAME DATABASE` — is rejected earlier and on **every** node, the leader included, with `SUPPORT_IS_DISABLED` (see the list of unsupported operations below); a follower never reports `TABLE_IS_READ_ONLY` for those. `SELECT` is allowed. Background write tasks are stopped, and any in-flight merges or moves on a node that just lost leadership are actively cancelled to bound the dual-writer window.
...
- `RENAME TABLE` and `RENAME DATABASE` — rejected on all `leader_election` tables. The lease path is fixed at startup and there is no protocol to broadcast a new name to the followers, so even under the default `Atomic` database (where the rename would otherwise be a metadata-only operation, because the data path is `UUID`-stable) the followers would keep tracking the old path. This applies both to renaming the table itself and to renaming a database that contains a `leader_election` table. Drop and recreate the table to rename it.
- `OPTIMIZE ... DRY RUN` — rejected on all nodes with `SUPPORT_IS_DISABLED`. It runs a real merge and commits the temporary merged part on the table's disks, bypassing the admission-epoch fence of a regular `OPTIMIZE`, and there is no node-local scratch path for it yet. Use a regular `OPTIMIZE` on the leader.
- `SYSTEM UNFREEZE` of a snapshot whose table is not attached on the node executing it — rejected with `SUPPORT_IS_DISABLED`. `SYSTEM UNFREEZE` receives only an on-disk path, so with the table gone there is no lease to check and deleting the shared snapshot could destroy a backup owned by the current leader. `FREEZE` marks its snapshots for this. Use `ALTER TABLE ... UNFREEZE WITH NAME` on the leader instead.
...
- [`leader_election`](/reference/settings/merge-tree-settings) — enable leader election for this table.
- [`leader_election_heartbeat_interval`](/reference/settings/merge-tree-settings) — how often the leader renews its lease.
- [`leader_election_session_timeout`](/reference/settings/merge-tree-settings) — when followers consider the lease expired and try to take over. Must be at least `3 * leader_election_heartbeat_interval`.
