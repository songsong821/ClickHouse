...
Hypothetical projections defined with [`CREATE HYPOTHETICAL PROJECTION`](/reference/statements/hypothetical-projection#create-hypothetical-projection) are candidates too. For a normal (sorted) projection the estimate builds the projection's primary index in memory from the parts the query would read, using the projection's own `WITH SETTINGS` granularity, prunes it with the query predicate exactly as a materialized projection would be pruned, and reports the marks and rows the projection read would touch, a signed `skip_ratio` (negative when the projection would read *more* than the base table) and a `verdict`: whether the optimizer would actually switch to it. The verdict follows the optimizer's rule: a projection wins when it reads fewer marks than the base table, or the same number while delivering the rows in the order an outer `ORDER BY` needs. A projection that cannot prune the predicate but does serve the `ORDER BY` is therefore estimated too, at all of its marks. Aggregate projections, projections with a `WHERE` clause and projections carrying their own skip indexes (`WITH SETTINGS add_minmax_index_*`) are listed with `status: not_applicable` and are not estimated yet, as is a projection that does not provide every column the query reads: the planner could still use such a projection to filter the base table's parts, and that path is not estimated. `force_optimize_projection`, `force_optimize_projection_name` and `preferred_optimize_projection_name` are ignored: the first would make planning fail before any candidate is evaluated, and the others only narrow which *existing* projection the planner may pick. The mark count a projection would have is derived from the parent part's sizes of the projection's columns, so with adaptive granularity (`index_granularity_bytes` != 0) it is an estimate rather than an exact count. No `est_bytes` is reported for a projection: its granules hold different rows than the base table's, so the base-table figure cannot be scaled into one. A projection whose definition no longer applies to the table — a dropped column, or a setting change that disables the features it needs — is reported with that reason instead.
