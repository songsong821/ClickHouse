...
The server successfully detected this situation and will download merged part from the replica to force the byte-identical result.
    ...
    M(AdaptiveAggregationPressureSweeps, "How many times the adaptive aggregation drained staged records early because of memory pressure, at production time. The finish drain of what was still staged when the input ended is counted by `AdaptiveAggregationFinishDrains` instead, so this event alone proves the production-time valve ran.", ValueType::Number) \
    M(AdaptiveAggregationPressureDrainedRecords, "How many staged records the adaptive aggregation drained early under memory pressure, at production time; the records the finish drain converted are counted by `AdaptiveAggregationFinishDrainedRecords` instead.", ValueType::Number) \
    M(AdaptiveAggregationFinishDrains, "How many times the adaptive aggregation's finish drain ran: the conversion, when the merge goes external, of the staged records that were still enqueued when the input ended.", ValueType::Number) \
    M(AdaptiveAggregationFinishDrainedRecords, "How many staged records the adaptive aggregation's finish drain converted into disk-mergeable form when the input ended.", ValueType::Number) \
    ...
    M(AdaptiveAggregationSharedTableSpillsBeforeTail, "How many times a pressure sweep of the adaptive aggregation wrote its shared drain table out before draining a tail into it, because the residue it held and the tail together would have reached the part bound; counted in `AdaptiveAggregationSharedTableSpills` too.", ValueType::Number) \
