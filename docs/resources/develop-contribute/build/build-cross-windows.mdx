...
:::note
The Windows port is **not finished**. CI links `clickhouse.exe` on every pull request and then
executes it under Wine (see [CI](#ci)) - `clickhouse-local` over a trivial query, which is as
far as the port has been run. Nothing has been run on a real Windows machine, and no test suite
runs against it. [Remaining work](#remaining-work) is an inventory of what is left, by
subsystem.
:::
...
**Run more of it.** CI executes `clickhouse.exe local --query "SELECT 1"` under Wine on every
pull request - the startup path and one pipeline. Everything past that is still unexecuted: the
client against a real server, `clickhouse-local` over actual files, any of the test suites, and
the binary on a real Windows machine rather than on Wine. Note for a development host that is
not `x86_64`: an `x86_64` PE needs an `x86_64` host, or an emulator such as FEX, which Wine on
ARM does not ship.
...
**No symlink aliases.** On Linux the install tree offers `clickhouse-client`, `clickhouse-local`
and the rest as extensionless symlinks to `clickhouse`; next to `clickhouse.exe` such a symlink
cannot resolve, so none are generated or installed on Windows. The mode is chosen the multi-call
way - `clickhouse.exe client`, `clickhouse.exe local` - or by copying the binary to a name such
as `clickhouse-local.exe`: the dispatcher reads the mode from `argv[0]` with the directory and
the case-insensitive `.exe` suffix stripped.
...
**A POSIX call and its Windows counterpart may not accept the same descriptors.** The first
time the binary was executed rather than merely linked, it threw before printing anything:
`getTerminalSize` read the console size from standard input, because `ioctl(TIOCGWINSZ)`
answers on any descriptor of the terminal. Its Windows counterpart does not - the size lives on
the console's *screen buffer*, which only an output handle has - so it failed with
`ERROR_INVALID_HANDLE` by construction. Nor is `_isatty` a test for "is a console": it is true
for every character device, `NUL` and the console input handle included, so the only usable
predicate was the console call itself succeeding. Expect more of this shape: it compiles, it
mirrors the POSIX code, and it is wrong.
...
installs `mingw-w64`. The job publishes no artifact: `clickhouse.exe` is not something to hand
to anyone yet.

The job then **runs** what it built, under Wine: `clickhouse.exe --version` and
`clickhouse.exe local --query "SELECT 1"`. That is why this cross-build, alone among the ones
here, runs on an `amd64` runner - an `x86_64` PE needs an `x86_64` host, and the ARM build of
Wine ships no `x86` emulator - and why `binary-builder` installs `wine64` on `amd64`.
`clickhouse-local` answering a trivial query covers the whole startup path: configuration, the
temporary data directory, the thread pools and one query pipeline. Everything the port
implements for Windows - the console queries, the socket and file-mapping wrappers,
`setThreadName`, the loopback `WakeupFd` - was written against the documentation and had never
been executed before this step existed, so this is where those assumptions are checked.
