...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.9"},{"label": "1"},{"label": "新增设置，用于在 Cascades 优化器中考虑将部分聚合下推到 join 之下（立即聚合）。"}]}]} />

在 Cascades 优化器中，将部分聚合下推到 join 之下 (立即聚合) 作为一种基于代价的备选方案纳入考虑。
仅在同时设置 `enable_cascades_optimizer = 1` 和 `make_distributed_plan = 1` 时生效。
  ...
  rows={[
{
  ...
  items: [
    ...
    {
      label:
        "在与早期版本 `compatibility` 的情况下，仍保留对估算大型集合 `IN` 选择性成本的限制：先前的值被有意设置为与新值相同，从而不会恢复无上限的估算——该估算可能会为单个查询的计划阶段增加数百毫秒。"
    }
  ]
},
{
  ...
  items: [
    ...
    {
      label:
        "新增设置，用于限制估算大型集合 `IN` 选择性时的成本：超过此限制后，估算器会使用集合大小及其边界范围，而非精确范围。先前的值被有意设置为与新值相同，从而在与早期版本 `compatibility` 时仍保留该限制，而不会恢复无上限的估算——该估算可能会为单个查询的计划阶段增加数百毫秒。"
    }
  ]
},
{
