...
The data does not have to be local: a URL can be used in place of a file name, and the URL scheme picks the matching table engine (`http://` and `https://` are read like the [url](/reference/functions/table-functions/url) function, `s3://` like the [s3](/reference/functions/table-functions/s3) function, `file://` like the [file](/reference/functions/table-functions/file) function):
...
When `--config-file` is not given, `clickhouse-local` looks for a configuration file in the following locations and uses the first one that exists:

1. `./config.xml`, `./config.yaml`, `./config.yml`
2. `./clickhouse-local.xml`, `./clickhouse-local.yaml`, `./clickhouse-local.yml`
3. `$HOME/.clickhouse-local/config.xml`, `$HOME/.clickhouse-local/config.yaml`, `$HOME/.clickhouse-local/config.yml`
4. `/etc/clickhouse-local/config.xml`, `/etc/clickhouse-local/config.yaml`, `/etc/clickhouse-local/config.yml`

A configuration file can be written either in XML or in YAML, in the same format as for ClickHouse server - see ["Configuration Files"](/operations/configuration-files).
...
`clickhouse-local` does not use the configuration of a ClickHouse server installed on the same
host - it looks for its own main configuration file and uses the first of these that exists:

1. the path passed with `--config-file`;
2. `./config.xml`;
3. `./clickhouse-local.xml`, `./clickhouse-local.yaml` or `./clickhouse-local.yml`;
4. `~/.clickhouse-local/config.xml`, `~/.clickhouse-local/config.yaml` or `~/.clickhouse-local/config.yml`;
5. `/etc/clickhouse-local/config.xml`, `/etc/clickhouse-local/config.yaml` or `/etc/clickhouse-local/config.yml`.

Just like for `clickhouse-server`, the `.xml`, `.yaml`, `.yml` and `.conf` files from the merge
directories next to the main configuration file are merged into it: `config.d` and `conf.d` for
`config.xml`, `clickhouse-local.d` and `conf.d` for `clickhouse-local.yaml`, and so on.

The merge directories of the current directory apply even when there is no main configuration file:
`./config.d` and `./conf.d` are merged into a minimal configuration embedded in the binary. This
makes a directory self-describing - for example, with
...
every `clickhouse-local` started in that directory has the `datasets` disk, and no `config.xml`
