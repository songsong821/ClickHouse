...
Before editing a documentation page, inspect it for `AUTOGENERATED_START` and `AUTOGENERATED_END`. Do not edit content between these markers directly; update the generator's source of truth instead. For reference documentation under `docs/reference/`, the source of truth is the structured documentation in the defining ClickHouse source registration, such as `FunctionDocumentation` or a `DECLARE` doc string. Do not update generated localized output directly. Direct changes to generated regions are reserved for the dedicated documentation autogeneration workflow and PRs which make changes to these sections without the `pr-autogenerated-docs` label will fail the docs check.
